...
### 0. Local Prefetch Cache Check (Token & Quota Optimization)
Before invoking external MCP servers or web scrapers:
- Compute or inspect the target folder `outputs/clients/{company-slug}/`.
- If `benchmark.json`, `places.json`, or `seo.json` already exist in that directory and are fresh, read and populate those sections directly from the local files.
- Bypass corresponding remote calls (`places-enrichment`, `seo-analyst`, `competitor-benchmark`) whenever local cached telemetry is available.
...
3. **Competitor Benchmarking (Step N+3)**:
   - Invoke the `competitor-benchmark` skill with company name, category/industry, and location.
   - Identify 2–3 local competitors with the highest review counts.
   - Pull reviews, rating, DR, and estimated traffic; compute averages.
   - Populate `competitor_benchmark` in the company profile for comparative scoring.
