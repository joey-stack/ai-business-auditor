...
class SettlementCoordinator:
    ...
    def compute_net_obligations(self) -> Dict[str, int]:
        ...
        for p in all_participants:
            ...
            # BUG: we also add what p is owed as if p owes it
            # (double-counting the reverse direction)
            for ob in pending:
                if ob.creditor == p:
                    total_p_owes += ob.amount_cents  # WRONG: creditor doesn't owe this
    ...
    def execute_settlement_cycle(self) -> SettlementResult:
        ...
        for participant, net_amount in net_amounts.items():
            if net_amount < 0:
                ...
                balance = self.ledger.balance(participant)
                if balance + net_amount >= 0:  # Can cover the obligation
                    ...
                    self.journal.append(
                        "SETTLEMENT_FAILED",
                        {"participant": participant, "net_amount": net_amount,
                         "balance": balance, "cycle": cycle_id},
                        source_node=self.node_id,
                    )
            elif net_amount > 0:
